...
$customPostType = array('works'); //custom postの処理に指定
$str_html="";
$str_html .= '<nav class="pankz ka2-container"><ul class="breadcrumb" itemscope itemtype="http://schema.org/BreadcrumbList">';
if (!is_front_page()) {
  ...
  if ( is_category() || is_singular('post') ) {//カテゴリー又は記事詳細
  ...
  }elseif( is_tax() || is_singular($customPostType)) {//カスタム投稿のsingle
    $post_type_obj = get_post_type_object($post->post_type);
    $archive_title = $post_type_obj ? $post_type_obj->label : '事例';
    $archive_link = get_post_type_archive_link($post->post_type);
    $str_html .= "<li class='breadcrumb-item' itemprop='itemListElement' itemscope itemtype='http://schema.org/ListItem'><a itemprop='item' href='".$archive_link."'>".'<span itemprop="name">'.$archive_title."</span></a><meta itemprop='position' content='3' /></li>";
    $str_html .= '<li class="breadcrumb-item" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">';
    if( is_single() ){
    ...
    $str_html .= '<meta itemprop="position" content="4" /></li>';
    if ( is_single() ) {
      $str_html .= $separator;
      $str_html .= "<li  class='breadcrumb-item' itemprop='itemListElement' itemscope itemtype='http://schema.org/ListItem'><a itemprop='item' href='".get_the_permalink()."'>".'<span itemprop="name">'.get_the_title()."</span></a><meta itemprop='position' content='5' /></li>";
    }
  }elseif(is_archive()) {//アーカイブページ
    $str_html .= "<li  class='breadcrumb-item' itemprop='itemListElement' itemscope itemtype='http://schema.org/ListItem'><a itemprop='item' href='".get_post_type_archive_link( $post->post_type )."'>".'<span itemprop="name">'.post_type_archive_title( '', false )."</span></a><meta itemprop='position' content='3' /></li>";
  }
